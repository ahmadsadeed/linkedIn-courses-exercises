...
API_KEY = ''  # replace with your own Google Maps API key

# add wifi info to request if available
payload = {'considerIp': 'true'}
try:
    cmdout = subprocess.check_output(['airport', '-s']).decode(encoding='utf_8')
    macs = re.findall(r'(?:[0-9a-fA-F]:?){12}', cmdout)
    signals = re.findall(r'', cmdout)
    channels = re.findall(r'', cmdout)
